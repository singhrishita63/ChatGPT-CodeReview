...
const parseHunkHeader = (hunkHeader?: string) => {
  ...
  const oldCount =
    oldCountValue === undefined ? 1 : Number(oldCountValue);

  const newStart = Number(newStartValue);
  const newCount =
    newCountValue === undefined ? 1 : Number(newCountValue);
...
export const getChangedFiles = async (
  ...
  try {
    ...
    log.debug(
      'failed to detect previous bot review',
      err
    );
  ...
  try {
    ...
    log.debug(
      `failed to compare commits from ${comparisonBase}`,
      err
    );
  }
...
export const robot = (app: Probot) => {
  ...
  app.on(
    ...
    async (context) => {
      ...
      const target_label =
        process.env.TARGET_LABEL;

      if (
        target_label &&
        (
          !pull_request.labels?.length ||
          pull_request.labels.every(
            (label) =>
          ...
          for (
            ...
            if (
              ...
              if (!inlineComment) {
                ...
                log.error(
                  `Failed to locate inline review comment: ${
                    review.hunk_header ||
                    'missing hunk header'
                  }`
                );
